...
用法：
  ...
  2. python scripts/test_local.py threads   # 只測 Threads
...
def main():
    platform = sys.argv[1] if len(sys.argv) > 1 else "threads"
    pid = int(sys.argv[2]) if len(sys.argv) > 2 else 1
    posts = P.load_posts()
    p = posts[pid]
    print(f"測試發布第 {pid} 則：{p['title']}")
    targets = ["threads", "fb", "ig"] if platform == "all" else [platform]
    for t in targets:
        try:
            P.PLATFORMS[t](p)
